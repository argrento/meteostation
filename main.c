...
uint16_t dataLines[8] = {GPIO_Pin_0, GPIO_Pin_1, GPIO_Pin_2,
...
const char *_wizFiCommands[_wizFiCommandsNumber] = {	
																"AT\r",
																"ATE0\r",								// Отключаем эхо команд
																...
																"AT+NCLOSEALL\r",
																"AT+NCTCP=144.76.102.166,80\r", //IP-адрес openweathermap.org
};
...
const char *weatherRequest = "\x1BS0GET /data/2.5/forecast/daily?q=Moscow&units=metric&cnt=7 HTTP/1.1\n"
...
int main(void){
	...
	sprintf(timeRTC, "%0.2d:%0.2d", RTC_CurrentTime.RTC_Hours, RTC_CurrentTime.RTC_Minutes);
  ...
  while (1) {
		...
		if (minutesCounter > 30) {
			...
			initWizFi220(wizFiRecallWeatherCommands, wizFiRecallWeatherCommandsNumber);
...
void writeByteToLCD(uint8_t crystalId, uint8_t byteType, uint8_t byte){
	...
	static uint8_t i;
	for (i = 0; i < 8; ++i) {
		GPIO_WriteBit(GPIOD, dataLines[i], ((byte & (mask << i)) != 0)?Bit_SET:Bit_RESET);
  }
	...
	while (message[i] != '\0') {
		for (charInMessage = 0; charInMessage < 8; charInMessage++) {
			...
			if ((firstPageNumber < 9) && (LCDXValue < 127)) {
				displayArray[firstPageNumber][LCDXValue] = asciiTable[message[i]][charInMessage] << offset;
			}
			if ((secondPageNumber < 9) && (offset != 0) && (LCDXValue < 127)) {
			  displayArray[secondPageNumber][LCDXValue] = asciiTable[message[i]][charInMessage] >> (8 - offset);
			}
    }
		i++;
	}
	...
	for (rowPartId = 0; rowPartId < 2; rowPartId++) {
		...
		for (colPartId = 0; colPartId < 2; colPartId++) { 
			for (byteInPage = 0; byteInPage < 8; byteInPage++) {
				...
				if ((firstPageNumber < 9) && (LCDXValue < 127)) {
					displayArray[firstPageNumber][LCDXValue] |= weatherImages[imageId][rowPartId*2 + colPartId][byteInPage] << offset;
				}
				if ((secondPageNumber < 9) && (offset != 0) && (LCDXValue < 127)) {
					displayArray[secondPageNumber][LCDXValue] = weatherImages[imageId][rowPartId*2 + colPartId][byteInPage] >> (8 - offset);
				}
			}
		}
	}
  ...
  uint8_t repeatCommand = 0;
    ...
    {}
	  ...
	  repeatCommand = 0;
		...
		do{
			// Ждем приема байта
			while((USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == RESET) && (TimingDelay != 0))
			{}
...
void parseAndSetWeather(int dayId, const char* weatherResponse){
	...
	sprintf(displayWeather, "%3.0d %3.0d %3.0d %3.0d", temperatures[0], temperatures[1], temperatures[2], temperatures[3]);
	...
	for (a = 0; a < 4; ++a) {
		...
		drawWeatherImage(iconXOffset + iconXStep * a, 40, convertIconNameToId(atoi(temp)));
		//iconNames[a] = 
	}
